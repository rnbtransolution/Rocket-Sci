...
import { generateOrderFlex, generateMatchSuccessFlex } from '../services/flexOrderService.js';

export function createWebhookRouter(client: messagingApi.MessagingApiClient): Router {
  const router = Router();

  router.post('/', (req: Request, res: Response): void => {
    // 1. Immediately return HTTP 200 OK (< 200ms) to satisfy LINE SLA and prevent webhook timeouts
    res.status(200).json({ status: 'ok' });

    const events: webhook.Event[] = req.body?.events || [];
    if (!events.length) return;

    // 2. Process events asynchronously in the background
    setImmediate(async () => {
      await Promise.all(
    ...
    if (betRegex.test(text) && userId) {
      ...
      if (match) {
        ...
        if (amount >= 50 && amount <= 50000) {
          ...
          console.log(`[Webhook] Created new Order #${newOrder.orderNumber} for ${displayName}`);
          return;
        }
      }
    }

    // Pattern 2: Accept bet command via text (e.g. "ต 9047", "ต9047", "รับ 9047", "#9047")
    ...
    if (matchRegex.test(text) && userId) {
      ...
      if (match) {
        ...
        try {
          ...
          console.log(`[Webhook] Text matched Order #${orderNumber} by ${displayName}`);
          return;
        } catch (err: any) {
